...
struct SBP4_1D <: SBP_TYPE
  ...
  E::NTuple{3,SparseMatrixCSC{Float64,Int64}}
  M::Int64
end
...
5. M: Number of discretization points
"""
struct SBP4_VARIABLE_1D <: SBP_TYPE
...
"""
Definition of the 4th order SBP operator with constant coefficients
"""
function SBP4_1D(m::Int64)
  ...
  D1 = spdiagm(-2=>(1/12)*ones(m-2), -1=>(-8/12)*ones(m-1), 1=>(8/12)*ones(m-1), 2=>(-1/12)*ones(m-2))
  ...
  # D₂ = H⁻¹(-A + BS)
  D2 = spdiagm(-2=>(-1/12)*ones(m-2), -1=>(16/12)*ones(m-1), 0=>-30/12*ones(m), 1=>(16/12)*ones(m-1), 2=>(-1/12)*ones(m-2))
  m55 = -5/2
  s15 = 0  
  ...
  BS[m,m-4:m] = reverse(BS[1,1:5], dims=1)
  ...
  # Store the transformation matrices for constructing 2d operators
  E0 = δᵢⱼ(1,1,(m,m))
  Em = δᵢⱼ(m,m,(m,m))
  E = I(m), E0, Em
  
  SBP4_1D(H, D1, (D2,D2c), S, E, m)
end

"""
Definition of the variable SBP operator
"""
function SBP4_VARIABLE_1D(N::Int64, c::AbstractVector{Float64})
  M = 78+(N-12)*5;  
  # Declare the variables
  R = spzeros(Float64, M)
  ...
  SBP4_VARIABLE_1D(c, H, D2, S, E, N)
end

"""
Function to convert the 1D operators to higher dimensions
"""
function δᵢⱼ(i::Int64, j::Int64, mn::Tuple{Int64,Int64})  
  m, n = mn
