...
function 𝐊4!(𝒫, 𝛀::Tuple{DiscreteDomain, DiscreteDomain, DiscreteDomain, DiscreteDomain},  𝐪𝐫)
  ...
  B̂₂, B̃₂, 𝐇₁⁻¹₂, 𝐇₂⁻¹₂ = SATᵢᴱ(𝛀₂, 𝛀₃, [1; 0], [-1; 0], NonConformingInterface(); X=I(2))
  B̂₃, B̃₃, 𝐇₁⁻¹₃, 𝐇₂⁻¹₃ = SATᵢᴱ(𝛀₃, 𝛀₄, [0; -1], [0; 1], NonConformingInterface(); X=I(2))  
  
  h = 1/(max(m₁,m₂,m₃,m₄)-1)
  ζ₀ = 40/h
  𝐓ᵢ¹ = blockdiag((I(2)⊗I(2)⊗𝐇⁻¹₁)*(0.5*B̂₁*𝐓rᵢ¹ - 0.5*𝐓rᵢ¹'*B̂₁ - ζ₀*B̃₁), zero(𝐏₃.A), zero(𝐏₄.A))
  𝐓ᵢ² = blockdiag(zero(𝐏₁.A), (blockdiag(I(2)⊗𝐇₁⁻¹₂, I(2)⊗𝐇₂⁻¹₂))*(-0.5*B̂₂*𝐓qᵢ² + 0.5*𝐓qᵢ²'*B̂₂ - ζ₀*B̃₂), zero(𝐏₄.A))
  𝐓ᵢ³ = blockdiag(zero(𝐏₁.A), zero(𝐏₂.A), (blockdiag(I(2)⊗𝐇₁⁻¹₃, I(2)⊗𝐇₂⁻¹₃))*(0.5*B̂₃*𝐓rᵢ³ - 0.5*𝐓rᵢ³'*B̂₃ - ζ₀*B̃₃))
