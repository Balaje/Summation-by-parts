...
# Include only 9000 < x < 12000
x_start = round(Int64, 9000/dx)
x_end = round(Int64, 12000/dx)
# Include only z > -2000
_, z_min_index = findmin(abs.(Z[:,1].-(z[end]/2)))
_, z_max_index = findmin(abs.(Z[:,1].-(0)))

X = X[z_max_index:z_min_index, x_start:x_end]
Z = Z[z_max_index:z_min_index, x_start:x_end]
rho = rho[z_max_index:z_min_index, x_start:x_end]
vp = vp[z_max_index:z_min_index, x_start:x_end]
vs = vs[z_max_index:z_min_index, x_start:x_end]
...
for i = 1:lastindex(downSamplings)
  ...
  filename = "./examples/MarmousiModel/marmousi2_crop_x_"*string(x_start)*"_"*string(x_end)*"_z0_"*string(z_max_index)*"_"*string(z_min_index)*"_downsampled_"*string(d)*".mat";
...
############### ################## 
vars = matread("./examples/MarmousiModel/marmousi2_crop_x_"*string(x_start)*"_"*string(x_end)*"_z0_"*string(z_max_index)*"_"*string(z_min_index)*"_downsampled_"*string(d)*".mat");
vp = vars["vp"]
vs = vars["vs"]
X = vars["X"]
Z = vars["Z"]
rho = vars["rho"]
plt1_1 = contourf(X[1,:]/1000, Z[:,1]/1000, vp/1000, lw=0.1, size=(800,400), bottom_margin=8*Plots.mm, xtickfontsize=12, ytickfontsize=12, top_margin=5*Plots.mm, xguidefontsize=12, yguidefontsize=12, left_margin=8*Plots.mm);
xlabel!(plt1, "Distance (km)");
ylabel!(plt1, "Depth (km)");
title!(plt1, "Cₚ (km/s)")

plt2_1 = contourf(X[1,:]/1000, Z[:,1]/1000, vs/1000, lw=0.1, size=(800,400), bottom_margin=8*Plots.mm, xtickfontsize=12, ytickfontsize=12, top_margin=5*Plots.mm, xguidefontsize=12, yguidefontsize=12, left_margin=8*Plots.mm);
xlabel!(plt2, "Distance (km)");
ylabel!(plt2, "Depth (km)");
title!(plt2, "Cₛ (km/s)")

plt3_1 = contourf(X[1,:]/1000, Z[:,1]/1000, rho/1000, lw=0.1, size=(800,400), bottom_margin=8*Plots.mm, xtickfontsize=12, ytickfontsize=12, top_margin=5*Plots.mm, xguidefontsize=12, yguidefontsize=12, left_margin=8*Plots.mm);
