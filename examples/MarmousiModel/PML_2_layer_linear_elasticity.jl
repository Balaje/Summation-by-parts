...
using MAT
vars1 = matread("./examples/MarmousiModel/marmousi2_crop_x_7206_9608_z0_1_1401_downsampled_5.mat");
X₁ = vars1["X_e"]
Z₁ = vars1["Z_e"]
x₁ = X₁[1,:]
z₁ = Z₁[:,1]
...
vars2 = matread("./examples/MarmousiModel/marmousi2_crop_x_7206_9608_z0_1401_2801_downsampled_5.mat");
X₂ = vars2["X_e"]
Z₂ = vars2["Z_e"]
x₂ = X₂[1,:]
z₂ = Z₂[:,1]
...
const σ₀ʰ = 4*(√(max(maximum(vp₁), maximum(vp₂))))/(2*δ)*log(10^4) #cₚ,max = 4, ρ = 1, Ref = 10^-4
...
"""
function σₕ(x)
  if((x[2] ≈ 0.9*Lₕ) || (x[2] > 0.9*Lₕ))
    return σ₀ʰ*((x[2] - Lₕ)/δ)^3  
  elseif( (x[2] ≈ -0.9*Lₕ) || (x[2] < -0.9*Lₕ) )
